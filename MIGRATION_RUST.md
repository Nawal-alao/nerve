...
|---|---|---|
| Bootstrap | `nerve/app.py` | MatuiApp, routage splash → login → chat, boucle d'app |
| Config | `nerve/config.py` | chemins, clés `config.json`, chiffrement du store (Fernet), clé de récupération |
| Comptes multi | `nerve/accounts.py` | `accounts.json` + tokens dans le keyring système |
| Couche Matrix | `nerve/matrix_client.py` | wrapper sur `matrix-nio` : login, sync, envoi, SAS, upload |
...
| Sidebar | `nerve/sidebar.py` | panneau contextuel droit |
| Formatage | `nerve/formatting.py` | couleurs par émetteur, markdown inline, regex URL |
| Thèmes | `nerve/themes.py` | tokens de couleur (opencode / matrix_green) |
| Images | `nerve/image_renderer.py` | Kitty / Sixel / placeholder |
| Notifications | `nerve/notifications.py` | `notify-send` |
| Widgets | `nerve/widgets.py` | helpers UI (figlet-like banner, …) |
...
- `config.rs` (chemins `~/.config/nerve`), `accounts.rs` + `keyring`.
...
- `ui/splash.rs` : bloc ASCII `NERVE` fixe + révélation colonne par colonne + tagline typée (`tachyonfx` ou timers custom, **mêmes cadeaux**). Skip via `enter`/`esc`, auto after ~3s.
...
- Versioning : `nerve 0.3.x` (Rust) reprend le numéro après la migration complète.

---

## 8. Première étape immédiate (prochaine action)

1. `cargo init` du workspace dans `/home/nawalalao/nerve` (ou sous-dossier `rust/`).
