...
from nerve.notifications import is_enabled, notify, CONFIG_FILE
...
class TestNotify:
    ...
    def test_silent_when_disabled(self, tmp_path: Path) -> None:
        ...
        config.write_text(json.dumps({"notifications_enabled": False}))
        with patch("nerve.notifications.CONFIG_FILE", config):
            with patch("nerve.notifications.subprocess.Popen") as mock_popen:
    ...
    def test_silent_when_no_notify_send(self, tmp_path: Path) -> None:
        ...
        with patch("nerve.notifications.CONFIG_FILE", config):
            with patch("nerve.notifications.shutil.which", return_value=None):
                with patch("nerve.notifications.subprocess.Popen") as mock_popen:
                    notify("Room", "Alice", "Hello")
                    mock_popen.assert_not_called()
    ...
    def test_calls_notify_send_when_available(self, tmp_path: Path) -> None:
        ...
        with patch("nerve.notifications.CONFIG_FILE", config):
            with patch("nerve.notifications.shutil.which", return_value="/usr/bin/notify-send"):
                with patch("nerve.notifications.subprocess.Popen", mock_popen):
