...
from shelltrix.notifications import is_enabled, notify, CONFIG_FILE
...
class TestIsEnabled:
    ...
    def test_reads_config_value(self, tmp_path: Path) -> None:
        ...
        with patch("shelltrix.notifications.CONFIG_FILE", config):
...
class TestNotify:
    ...
    def test_silent_when_disabled(self, tmp_path: Path) -> None:
        ...
        config.write_text(json.dumps({"notifications_enabled": False}))
        with patch("shelltrix.notifications.CONFIG_FILE", config):
            with patch("shelltrix.notifications.subprocess.Popen") as mock_popen:
    ...
    def test_silent_when_no_notify_send(self, tmp_path: Path) -> None:
        ...
        with patch("shelltrix.notifications.CONFIG_FILE", config):
            with patch("shelltrix.notifications.shutil.which", return_value=None):
                with patch("shelltrix.notifications.subprocess.Popen") as mock_popen:
                    notify("Room", "Alice", "Hello")
                    mock_popen.assert_not_called()
    ...
    def test_calls_notify_send_when_available(self, tmp_path: Path) -> None:
        ...
        with patch("shelltrix.notifications.CONFIG_FILE", config):
            with patch("shelltrix.notifications.shutil.which", return_value="/usr/bin/notify-send"):
                with patch("shelltrix.notifications.subprocess.Popen", mock_popen):
        ...
        with patch("shelltrix.notifications.CONFIG_FILE", config):
            with patch("shelltrix.notifications.shutil.which", return_value="/usr/bin/notify-send"):
                with patch("shelltrix.notifications.subprocess.Popen", mock_popen):
                    ...
                    body_arg = [a for a in args if len(a) >= 200]
                    assert body_arg == ["x" * 200]
