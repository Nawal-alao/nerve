...
- Timeline: **conversationally grouped** — consecutive messages from the same sender collapse into a block with a single `› Vous` / `‹ Name` indicator; a dimmed `HH:MM ────` separator appears after ~5 min of silence. Inline markdown (bold, italic, code, strikethrough) and inline image previews (Kitty/Sixel graphics protocol)
